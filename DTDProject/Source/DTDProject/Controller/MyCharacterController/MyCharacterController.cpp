...
#include "UI/GoodResultUI.h"
#include "UI/BadResultUI.h"
#include "UI/ShopUI.h"
...
void AMyCharacterController::OnPossess(APawn* InPawn)
{
	Super::OnPossess(InPawn);

	AMyRobo* Robo = Cast<AMyRobo>(InPawn);
	if (Robo)
	{
		// 수면 도달 델리게이트 바인딩
		Robo->OnSurfaced.AddUObject(this, &AMyCharacterController::OnRoboSurfaced);
		// 산소 고갈 델리게이트 바인딩
		if (URoboComponent* RoboComponent = Robo->FindComponentByClass<URoboComponent>())
		{
			RoboComponent->OnOxygenDepleted.AddUObject(this, &AMyCharacterController::OnRoboOxygenDepleted);
		}
	}
	CurrentGameState = EPlayerGameState::Diving;
}

void AMyCharacterController::OnRoboSurfaced()
{
	if (CurrentGameState == EPlayerGameState::Diving)
	{
		ShowResultUI();
	}
}

void AMyCharacterController::ShowResultUI()
{
	CurrentGameState = EPlayerGameState::ShowResult;
	SetInputMode(FInputModeUIOnly());
	bShowMouseCursor = true;

	AMyRobo* Robo = Cast<AMyRobo>(GetPawn());
	if (!Robo || !Robo->GetInventoryComponent()) return;

	if (ResultUIClass && !ResultUIInstance)
	{
		ResultUIInstance = CreateWidget<UGoodResultUI>(this, ResultUIClass);
	}

	if (ResultUIInstance)
	{
		const TArray<FCaughtFishInfo>& CaughtFish = Robo->GetInventoryComponent()->GetCaughtFishList();
		//GoodResultUIInstance->ShowResult(CaughtFish);
		ResultUIInstance->AddToViewport();

		/*GoodResultUIInstance->OnConfirm.AddLambda([this, Robo]()
			{
				Robo->GetInventoryComponent()->SellAllFish();
				GoodResultUIInstance->RemoveFromParent();
				ShowShopUI();
			}
		);*/
	}
...
void AMyCharacterController::OnRoboOxygenDepleted()
{
	if (CurrentGameState == EPlayerGameState::Diving)
	{
		ShowBadResultUI();
...
void AMyCharacterController::ShowBadResultUI()
{
	CurrentGameState = EPlayerGameState::ShowResult;
	SetInputMode(FInputModeUIOnly());
	bShowMouseCursor = true;
	...
	AMyRobo* Robo = Cast<AMyRobo>(GetPawn());
	if (!Robo || !Robo->GetInventoryComponent()) return;
	if (BadResultUIClass && !BadResultUIInstance)
	{
		BadResultUIInstance = CreateWidget<UBadResultUI>(this, BadResultUIClass);
	}

	if (BadResultUIInstance)
	{
		BadResultUIInstance->AddToViewport();

	/*	BadResultUIInstance->OnConfirm.AddLambda([this, Robo]()
			{
				Robo->GetInventoryComponent()->SellAllFish();
				BadResultUIInstance->RemoveFromParent();
				ShowShopUI();
			});*/
	}
}

void AMyCharacterController::ShowShopUI()
{
	CurrentGameState = EPlayerGameState::Shop;

	if (ShopUIClass && !ShopUIInstance)
	{
		ShopUIInstance = CreateWidget<UShopUI>(this, ShopUIClass);
	}

	if (ShopUIInstance)
	{
		ShopUIInstance->AddToViewport();
	/*	ShopUIInstance->OnConfirm.AddUObject(this, &AMyCharacterController::ReturnToDivingState);*/
	}
}

void AMyCharacterController::ReturnToDivingState()
{
	if (ShopUIInstance)
	{
		ShopUIInstance->RemoveFromParent();
	}
